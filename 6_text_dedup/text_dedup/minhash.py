...
from utils.add_args import add_own_args
...
if __name__ == "__main__":
    ...
    # data_files = os.listdir(args.input_path)
    # data_files = [os.path.join(args.input_path, data_file) for data_file in data_files][:]
    ...
    if args.b is not None and args.r is not None:
        ...
        B, R = optimal_param(args.threshold, args.num_perm, args.false_positive_weight, args.false_negative_weight)
    
    logger.info(f"B is {B}, R is {R}")   
    logger.info(f'threshold is {args.threshold}, num_perm is {args.num_perm}') 
    ...
    with timer("Total"):
        ...
        with timer("MinHashing"):
            embedded = ds.map(
                ...
                num_proc=os.cpu_count() if args.num_proc==None else args.num_proc,
                with_indices=True,
                desc="Fingerprinting...",
            )
        ...
        with timer("Filtering"):
            ...
            final_data_duped = ds.filter(
                function=lambda record, idx: record["__cluster__"] != idx,
                with_indices=True,
                num_proc=os.cpu_count()
                if args.num_proc == None else args.num_proc,
                desc="Filtering clusters...",
            )


        with timer("Saving"):
            # filter_data = [{"text": d["text"]} for d in final_data]
            save_jsonl(final_data, args.output_path)
            # final_data.to_json(args.output_path, orient="records", lines=True, force_ascii=False)

            if args.debug:
                with open(os.path.join(args.output_path, "uf.pkl"), "wb") as f:
